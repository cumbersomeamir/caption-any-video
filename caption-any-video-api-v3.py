...
def process_video():
    ...
    output_video_path = f"video_with_captions_{unique_id}.mp4"  # Use unique output filename
    add_captions_to_video(video_path, transcription_data, output_video_path)

    # Step 5: Upload the final video with captions to S3
    s3_filename = f"videos/final_video_with_captions_{unique_id}.mp4"  # Ensure unique filename in S3
